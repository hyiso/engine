...
uint64_t PlatformViewOHOS::RegisterExternalTexture(int64_t texture_id)
{
  ...
  if (ohos_context_->RenderingApi() == OHOSRenderingAPI::kOpenGLES) {
    OH_NativeImage* native_image = OH_NativeImage_Create(texture_id, GL_TEXTURE_EXTERNAL_OES);
    if (native_image == nullptr) {
      FML_LOG(ERROR) << "PlatformViewOHOS::RegisterExternalTexture Error with OH_NativeImage_Create";
      return surface_id;
    }
    std::shared_ptr<OHOSExternalTextureGL> ohos_external_gl =
      std::make_shared<OHOSExternalTextureGL>(texture_id, native_image);
    ...
    ret = OH_NativeImage_SetOnFrameAvailableListener(native_image, listener);
    if (ret != 0) {
      FML_DLOG(ERROR) << "PlatformViewOHOS::RegisterExternalTexture Error with OH_NativeImage_SetOnFrameAvailableListener";
      return surface_id;
    }
    ret = OH_NativeImage_GetSurfaceId(native_image, &surface_id);
    if (ret != 0) {
      FML_LOG(ERROR) << "PlatformViewOHOS::RegisterExternalTexture Error with OH_NativeImage_GetSurfaceId";
      return surface_id;
    }
